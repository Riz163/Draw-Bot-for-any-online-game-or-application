...
def ditheroption(image, palettedata):
    print("Processing Image...")
    im = image
    fill_color = (255, 255, 255)
    if im.mode in ('RGBA', 'LA'):
    ...
    length = len(
        palettedata) - 3  # -3 for white because white is the background color (if not simply do click on white while calibrating)
    a = 0
    v = 0
    while a < length:
        getcoords()
...
def quantizeOption(image, palettedata):
    layers = 1 # too lazy to rewrite the drawing functions, Ill leave that for later
    im = image
    fill_color = (255, 255, 255)
    if im.mode in ('RGBA', 'LA'):
        background = Image.new(im.mode[:-1], im.size,
    ...
    dummy = Image.new('P', (16, 16))  # creates an image to put the color palette on
    dummy.putpalette(palettedata)
    image_quantized = image_halfresized.convert("RGB").quantize(palette=dummy, dither=False)  # dithers the image with the palette of
    # the dummy image using floyd-steinberg dithering
    image_quantized.save("img_quant.png")  # temporary save because pillow doesnt like mode P for getting the pixels
    ...
    while w < len(coordinates):
        ...
        ycoords.append(coordinates[w + 1])
        w += 2
    image_dithered = Image.open("img_quant.png").convert('RGB')
    x, y = image_dithered.size
    ...
    while a < length:
        ...
        v += 1
    # the data is ready now...
    os.remove("img_quant.png")  # hide the evidence O_o

    def drawQuantize1(e, c):  # this draws all the pixels of one color (e) divided by c for the layers
        cc = 3 * speeed
        while c < len(pixels[e]):

            if keyboard.is_pressed('q'):  # Failsafe
                break
            mouse.move(int(pixels[e][c] * pp + offset_x + int((canvas_x - width * pp) / 2)),
                ...
                if val == "1":
                    ...
                    while True:
                        try:
                            ...
                            image_bytes = io.BytesIO(response.content)  # gets the image from the url
                            image = Image.open(image_bytes).convert("RGBA")
                            ditheroption(image, palettedata)
                            break
                        except:
                            print("Failed Processing...")
                            print("Enter a valid url (and) or choose another image")
