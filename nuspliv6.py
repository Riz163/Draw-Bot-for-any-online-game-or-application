    ...
    while True:
        ...
        try:
            ...
            if im.mode in ('RGBA', 'LA'):
                background = Image.new(im.mode[:-1], im.size, fill_color) # idk how this works but it puts a white background
            ...
            width = int(width / pp) # pp is the brush size and this is needed because 1 pixel of the image cant be 1 pixel
            height = int(height / pp) # in the drawing if pp is greater than 1 because it would overlap everything
                                    # thus it makes the image smaller first and then when drawing clicks on every pixel
                                    # but as spaced out as the brush size is thick to leave no gaps or have it overlapping
            ...
            image_dithered = image_halfresized.quantize(palette=dummy)  # dithers the image with the palette of
            ...
            while w < len(coordinates):
                xcoords.append(coordinates[w])
                ycoords.append(coordinates[w + 1])
                w += 2
            ...
            def draw1(e, c): # this draws all the pixels of one color (e) divided by c for the layers
                cc = 3 * speeed
                while c < len(pixels[e]):
                    if keyboard.is_pressed('q'):  # Failsafe
                        break
                    mouse.move(int(pixels[e][c] * pp + offset_x + int((canvas_x - width * pp) / 2)), # similar to canny option
                               int(pixels[e][c + 1] * pp + offset_y + int((canvas_y - height * pp) / 2)), # but upscaling
                ...
                while z < layers:
                    ...
                    while f < len(xcoords):
                        if keyboard.is_pressed('q'):  # Failsafe
                            print("Drawing interrupted")
                            break
                        pyautogui.moveTo(xcoords[b], ycoords[b], 0) # selects the right color first
                        pyautogui.click()
                        time.sleep(0.01)
                        draw1(f, c)
                        f += 1
...
def ditheroptionblack():
    while True:
        value = input("Enter image url...\n(or b to go back)\n")
        if value == "b":
            break
        try:
            response = requests.get(f"{value}") # same as with colors...
            image_bytes = io.BytesIO(response.content)
            im = Image.open(image_bytes).convert("RGBA")
            ...
            image_halfresized = im.resize((width, height))  # resizes the image to always fit on the canvas
...
def calibrate(): # settings - Im using text files to store them and there is probably also a better way but yea...
    while True:
        ...
        if res == "1":
            ...
            while True:
                if keyboard.is_pressed('s'):
                    ...
                    print("Important - dont click on white !") # (if you do nothing bad happens)
